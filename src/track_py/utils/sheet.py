...
def get_cached_today_data(sheet_name, today_str, force_refresh=False):
    ...
    if not force_refresh and cache_key in _sheet_cache:
        ...
        if current_time - timestamp < _today_cache_timeout:
            logger.info(f"Using cached today data for {today_str} in sheet {sheet_name}")
            return data
    ...
    try:
        ...
        try:
            ...
            all_values_meta = sheet.get_values("A:A")  # Just get column A to find last row
            if all_values_meta:
                ...
                logger.info(f"Optimized fetch range: {fetch_range} (detected {last_row} rows)")
                all_values = sheet.get_values(fetch_range)
                logger.info(f"Fetched {all_values} rows for today data")
            else:
                all_values = []
        except Exception as range_error:
            logger.debug(f"Range optimization failed, using default range: {range_error}")
...
def invalidate_sheet_cache(sheet_name):
    ...
    today_keys_to_remove = [key for key in _sheet_cache.keys() if key.startswith(f"today_data_{sheet_name}_")]
...
async def get_daily_process_data(time_with_offset):
    ...
    try:
        ...
        all_values = await asyncio.to_thread(get_cached_today_data, target_month, today_str)
