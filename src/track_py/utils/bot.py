...
from src.track_py.utils.sheet import get_cached_worksheet, invalidate_sheet_cache, get_cached_sheet_data, convert_values_to_records, parse_amount
...
async def wait_for_background_tasks(timeout=30):
    ...
    while (
        _background_tasks or log_expense_queue or delete_expense_queue
            ...
            for target_month, expenses in grouped_by_month.items():
                try:
                    ...
                    for expense_data in expenses:
                        await send_error_notification(expense_data, month_error, LOG_ACTION)
        ...
        while delete_expense_queue:
            ...
            for target_month, expenses in grouped_by_month.items():
                try:
                    ...
                    logger.error(f"Error processing delete expenses for month {target_month}: {month_error}")
                    # Send error notifications for this month's expenses
                    for expense_data in expenses:
                        await send_error_notification(expense_data, month_error, DELETE_ACTION)
        ...
        for expense_data in expenses:
            ...
            try:
                ...
                if not all_values or len(all_values) < 2:
                    ...
                    await send_error_notification(expense_data, "Không có dữ liệu trong sheet này", DELETE_ACTION)
                    continue
                ...
                for i, r in enumerate(records):
                    ...
                    if row_date == entry_date and row_time == entry_time:
                        found_row = i + 2  # +2 because records start from row 2 (header is row 1)
                        break
                ...
                if found_row:
                    ...
                    await send_error_notification(expense_data, f"Không tìm thấy giao dịch: {entry_date} {entry_time}", DELETE_ACTION)
                    continue
                    
            except Exception as delete_error:
                logger.error(f"Error deleting expense {entry_date} {entry_time}: {delete_error}", exc_info=True)
                await send_error_notification(expense_data, f"Lỗi khi xóa: {delete_error}", DELETE_ACTION)
                continue
...
async def background_log_expense(entry_date, entry_time, amount, note, target_month, user_id, chat_id, bot_token, message_id):
...
async def background_delete_expense(entry_date, entry_time, target_month, user_id, chat_id, bot_token, message_id):
    ...
    try:
        ...
        logger.info(f"Queued expense deletion for user {user_id}. Queue size: {queue_position}")
